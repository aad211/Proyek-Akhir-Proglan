...
struct sbuku{
	int peminjam, waktu, index_rak, lembar;
	char bukunya[100];
	struct sbuku *lanjut;
};

struct srak *rawal = NULL;
struct srak *rakhir = NULL;

struct sbuku *bawal = NULL;
struct sbuku *bakhir = NULL;

void display_rak(){
	struct srak *ptr = rawal;
...
void display_buku(){
	struct sbuku *ptr = bawal;
	
	printf("\n[head] =>");
	while(ptr != NULL) {
		printf(" %s, %d, %d, %d, %d =>",ptr->bukunya, ptr->index_rak, ptr->lembar, ptr->peminjam, ptr->waktu);
		ptr = ptr->lanjut;
	}
	printf(" [null]\n");
}

void tambah_srak(char *nama){
	struct srak *baru = (struct srak*) malloc(sizeof(struct srak));
	
	strcpy(baru->raknya, nama);
...
void tambah_sbuku(char *nama, int peminjam, int waktu, int index_rak, int lembar){
	struct sbuku *baru = (struct sbuku*) malloc(sizeof(struct sbuku));
	
	strcpy(baru->bukunya, nama);
	baru->peminjam = peminjam;
	baru->waktu = waktu;
	baru->index_rak = index_rak;
	baru->lembar = lembar;
...
//char nama_user[100]; //belum dipakai, buat database data lengkapnya si user
char cari[100];
FILE *b;
FILE *c;
int pinjam;
...
int main (){
	...
	for (i=1; fscanf(b, "%[^\n]%*c\n", nama_rak)!=EOF; i++){
		tambah_srak(nama_rak);
		if (i%5==0){
			printf(".");
			Sleep(500);
		}
		if (i%25==0){printf("\b \b\b \b\b \b\b \b\b \b");}
		rak++;
	}
	fclose (b);
	...
	for (i; fscanf(b, "%d %d %d %d %[^\n]%*c\n", &pinjam, &waktu, &j, &lembar, nama_buku)!=EOF; i++){
		if (pinjam>=0){tambah_sbuku(nama_buku, pinjam, waktu, j, lembar);}
...
void daftar(){ //fungsi untuk mendaftar
	...
	fprintf(b, "%s ", cari);
	fclose(b);
	//scanf nama, ttl, alamat, dsb
	//nama lengkap, ttl, alamat lengkap, 
	/*b = fopen("datauser.bin", "a+"); //untuk menyimpan segala data user yang diperuntukkan untuk keamanan saat buku tidak dikembalikan (untuk saat ini belum dibutuhkan)
	b = fclose(b);*/
	system("cls");
